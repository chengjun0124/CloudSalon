...
app.filter("toLastConsumedBeautician", [function () {
    return function (input) {
        if (input == null)
            return "未使用";

        if (input.consumedServices.length > 0)
            return input.consumedServices[0].employeeNickName;
        else
            return "未使用";
    };
}]);
...
app.filter("toConsumedTime", [function () {
    return function (input) {
        if (input == null)
            return "0次";
        ...
        var count = 0;
        for (var i = 0; i < input.consumedServices.length; i++) {
            count += input.consumedServices[i].time;
        }
        return count + "次";
    };
}]);